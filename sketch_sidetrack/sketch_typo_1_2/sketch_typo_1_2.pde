...
void draw() {
  ...
  if (textTyped.length() > 0) {
    ...
    // lines
    stroke(150, 255 - mouseY*0.75, 255);
    strokeWeight(1.0);
    for (int i = 0; i < pnts.length; i++ ) {
      float l = 5 * mouseX*0.002;
      line(pnts[i].x-l, pnts[i].y-l, pnts[i].x+l, pnts[i].y+l);
    }

    // dots
    fill(150, mouseY*0.75, 255);
    noStroke();
    for (int i = 0; i < pnts.length; i++ ) {
      float diameter = 7;
      // on ervery second point
      if (i%2 == 0) {
        ellipse(pnts[i].x * (1 + mouseX*0.00002), pnts[i].y, diameter, diameter);
      }
    }
